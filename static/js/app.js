...
document.addEventListener('DOMContentLoaded', () => {
    initializeWebSocket();
    // Send button click
    document.getElementById('send-button').addEventListener('click', generateText);
...
function getSettings() {
    ...
    const randomnessEnabled = document.getElementById('toggle-randomness').checked;
    ...
    if (randomnessEnabled) {
        temperature = parseFloat(document.getElementById('temperature-value').value) || 0.7;
    ...
    if (penaltiesEnabled) {
        repetition_penalty = parseFloat(document.getElementById('repetition-penalty-value').value) || 1.0;
        frequency_penalty = parseFloat(document.getElementById('frequency-penalty-value').value) || 0.0;
        presence_penalty = parseFloat(document.getElementById('presence-penalty-value').value) || 0.0;
        repeat_last_n = parseInt(document.getElementById('repeat-last-n-value').value) || 64;
    }

    if (dryEnabled) {
        dry_allowed_length = parseInt(document.getElementById('dry-allowed-length').value) || 1;
        dry_base = parseFloat(document.getElementById('dry-base').value) || 2;
        dry_multiplier = parseFloat(document.getElementById('dry-multiplier').value) || 3;
        dry_range = parseInt(document.getElementById('dry-range').value) || 1024;
    }
    
    const num_show = parseInt(document.getElementById('num-show-value').value) || 12;
    const max_new_tokens = parseInt(document.getElementById('max-new-tokens-value').value) || 0;
...
async function generateText() {
    ...
    chatHistory.push({ role: 'assistant', content: '', partial: true, tokenSequence: [], id: chatHistory.length, chosenTokens: [] });
    updateChatDisplay();
...
async function replaceToken(message, position, token) {
    ...
    message.chosenTokens = message.chosenTokens.slice(0, position).concat([message.tokenSequence[position].find(([t, _, __]) => t === token)[0]]);
    message.content = message.chosenTokens.join('')
    message.tokenSequence = message.tokenSequence.slice(0, position).concat([message.tokenSequence[position].map(([t, p, c]) => [t, p, t === token])]);
    await generateResponse(chatHistory);
}
